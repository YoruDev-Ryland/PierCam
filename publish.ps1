...
# exist as a real file on disk — hence a small folder rather than a literal single file.
param([switch]$NoZip)
...
# there, so it trades 114 MB of permanent memory for disk. This program exists because SharpCap
# got heavy after a few days; it is not going to hand that back for a smaller download, and the
# zip claws most of the difference back anyway.
...
# The publish drops a PDB-less exe plus tools\ffmpeg.exe; nothing else should ship.
...
@"
PierCam $version — dusk-to-dawn timelapse for an observatory pier camera

WHAT YOU NEED
  Windows 10 or 11, 64-bit. No .NET install: the runtime is inside PierCam.exe.
...
RUNNING IT
  Unzip the whole folder and run PierCam.exe. Keep tools\ffmpeg.exe beside it — that is
  what writes the video.
...
FIRST RUN
  ...
  and set your site coordinates under SITE & ROOF (or import them from NINA). Everything on
  that page is saved per-user under %AppData%\PierCam and nothing is baked into the build.
...
LICENCE
  See THIRD-PARTY.txt for ffmpeg, which is GPL and is redistributed here under its own terms.
...
FFmpeg — tools\ffmpeg.exe
  Version 9.0.1, the "essentials" build published by Gyan Doshi (www.gyan.dev), configured
  with --enable-gpl --enable-version3. It is therefore covered by the GNU General Public
  License version 3 or later, NOT by whatever terms cover PierCam itself.
  ...
  Source code for this build, and the licence text, are published alongside the binaries at
...
$exe = Get-Item "$stage\PierCam.exe"
"`n  PierCam.exe        {0,8:N1} MB" -f ($exe.Length / 1MB)
"  tools\ffmpeg.exe   {0,8:N1} MB" -f ((Get-Item "$stage\tools\ffmpeg.exe").Length / 1MB)
"  files in drop:     {0}" -f (Get-ChildItem $stage -Recurse -File).Count

if (-not $NoZip) {
    $zip = "$root\dist\$name.zip"
    Compress-Archive -Path "$stage\*" -DestinationPath $zip -CompressionLevel Optimal
    "  zip                {0,8:N1} MB  ->  $zip" -f ((Get-Item $zip).Length / 1MB)
}
